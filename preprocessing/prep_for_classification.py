...
def sanitize_label_for_filename(label: list | str) -> str:
    """
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        default=["superiorfrontal-lh|superiorfrontal-rh"],
        help="Regular expression for the parcels to include",
    )
    parser.add_argument(
        "--all_parcels",
        default=False,
        action="store_true",
        help="Whether to include all parcels (ignores parcel_regex)",
    )
    parser.add_argument(
        "--tmin",
        type=float,
        default=-0.2,
        help="Start time before event to include in epochs",
    )
    parser.add_argument(
        "--tmax",
        type=float,
        default=2.0,
        help="End time after event to include in epochs",
    )
    ...
    fwd_fsaverage_path = (
        fs_subjects_dir / "fsaverage" / "bem" / "fsaverage-oct-6-src.fif"
    )
    ...
    if not args.all_parcels:
        ...
        labels = [
            ...
            for label in mne.read_labels_from_annot(
                "fsaverage", parc="aparc", subjects_dir=fs_subjects_dir
            )
        ]
    ...
    for label in labels:
        ...
        for subject in subjects:
            ...
            for idx, recording_name in enumerate(recording_names):
                logging.info(f"Processing recording {recording_name}, subject {subject}, label {label}")
                subject_session_info = subject_info[recording_name]

                fif_file_path = list(
                    (subject_meg_path / "MEG" / recording_name / "files").glob("*.fif")
                )[0]
                ...
                fwd = mne.read_forward_solution(
                    fs_subjects_dir / subject / "bem" / fwd_fname
                )
                ...
                morph_subject_path = (
                    fs_subjects_dir / subject / "bem" / f"{subject}-oct-6-src-morph.h5"
                )
                X_tmp = morph_stcs_label(
                    morph_subject_path, stcs, fs_subjects_dir, label
                )
                ...
                logging.info(f"FINISHED processing recording {recording_name}, subject {subject}, label {label}")
